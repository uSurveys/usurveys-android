...
import com.usersneak_internal.utils.common.Assert;
...
public class RequestStatus<T> {
  ...
  public static <T> RequestStatus<T> successWithDebugToast(T result) {
    Toast.makeText(AppContext.get(), "Success", Toast.LENGTH_SHORT).show();
  ...
  public static <T> RequestStatus<T> errorWithDebugToast(Throwable error) {
    if (BuildConfig.DEBUG) {
      Toast.makeText(AppContext.get(), "failure: " + error.getMessage(), Toast.LENGTH_SHORT).show();
    }
    return error(error);
  }
